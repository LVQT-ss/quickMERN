...
export default function UserProfilePage() {
  ...
  const save = async () => {
    try {
      setSaving(true)
      const token = getToken()
      const u = await api.users.update(id, { username, email, bio, role }, token)
      setUser(u)
  ...
  return (
    <div className="min-h-screen p-6 max-w-2xl mx-auto space-y-3">
      ...
      <button disabled={saving} onClick={save} className="rounded bg-blue-600 text-white px-3 py-2">{saving ? 'Saving...' : 'Save'}</button>
    </div>
  )
}
