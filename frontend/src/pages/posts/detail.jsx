...
export default function PostDetailPage() {
  ...
  const images = useMemo(() => (post && (post.PostImages || post.postImages || post.post_images || [])) || [], [post])
  ...
  return (
    <div className="min-h-screen p-6 max-w-3xl mx-auto space-y-4">
      ...
      <div className="flex items-center gap-2">
        <button onClick={toggle} className="rounded bg-blue-600 text-white px-3 py-1">Like</button>
        <span>{count}</span>
      </div>
      <div>
        <h2 className="text-xl font-semibold mb-2">Sections</h2>
        {(post.PostSections || post.postSections || post.post_sections || []).map(s => (
          <div key={s.id} className="border rounded p-3 mb-2">
            ...
            <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-2">
              {images.filter(img => (img.sectionId ?? img.section_id ?? null) === s.id)
                .sort((a,b)=> (a.orderIndex ?? a.order_index ?? 0) - (b.orderIndex ?? b.order_index ?? 0))
                .map(img => (
                  <div key={img.id} className="border rounded p-2">
                    <img src={(img.imageUrl ?? img.image_url) || ''} alt={img.caption || 'image'} className="w-full h-auto rounded" />
                    {img.caption && <div className="text-xs text-gray-600 mt-1">{img.caption}</div>}
                  </div>
                ))}
            </div>
          </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {images.filter(img => (img.sectionId ?? img.section_id ?? null) == null)
            .sort((a,b)=> (a.orderIndex ?? a.order_index ?? 0) - (b.orderIndex ?? b.order_index ?? 0))
            .map(img => (
              <div key={img.id} className="border rounded p-2">
                <img src={(img.imageUrl ?? img.image_url) || ''} alt={img.caption || 'image'} className="w-full h-auto rounded" />
